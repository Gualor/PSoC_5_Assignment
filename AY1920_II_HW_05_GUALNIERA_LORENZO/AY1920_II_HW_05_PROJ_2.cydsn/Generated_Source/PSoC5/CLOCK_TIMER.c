...
void CLOCK_TIMER_Stop(void) 
{
    ...
    CLOCK_TIMER_CLKEN &= (uint8)(~CLOCK_TIMER_CLKEN_MASK);
	CLOCK_TIMER_CLKSTBY &= (uint8)(~CLOCK_TIMER_CLKSTBY_MASK);
}
...
{
    if ((CLOCK_TIMER_CLKEN & CLOCK_TIMER_CLKEN_MASK) != 0u)
    {
    ...
    if (clkDivider != oldDivider)
    {
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CLOCK_TIMER_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLOCK_TIMER_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    CLOCK_TIMER_MOD_SRC |= modeBitMask & (uint8)CLOCK_TIMER_MODE_MASK;
}
...
{
    CLOCK_TIMER_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLOCK_TIMER_MODE_MASK));
}
...
{
    return CLOCK_TIMER_MOD_SRC & (uint8)(CLOCK_TIMER_MODE_MASK);
}
...
void CLOCK_TIMER_SetSourceRegister(uint8 clkSource) 
{
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLOCK_TIMER_MOD_SRC |= CYCLK_SSS;
        CLOCK_TIMER_MOD_SRC =
            (CLOCK_TIMER_MOD_SRC & (uint8)(~CLOCK_TIMER_SRC_SEL_MSK)) | clkSource;
    }
